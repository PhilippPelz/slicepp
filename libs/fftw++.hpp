...
: fftw(2*nx*ny,sign), nx(nx), ny(ny) {Setup(in,out);}
	...
	fftw_plan Plan(Complex *in, Complex *out) {
		return fftw_plan_dft_2d(nx,ny,(fftw_complex *) in,(fftw_complex *) out, sign,effort);
	}

	static QSTEM::ComplexArray2DPtr fftshift(QSTEM::ComplexArray2DPtr data) {
#define SWAP(a,b)       {t=a; a=b; b=t;}
		...
		for( ix=0; ix<nx; ix++)
			for( iy=0; iy<iymid; iy++) {
				SWAP( data[ix][iy], data[ix][iy+iymid] );
			}

		for( ix=0; ix<ixmid; ix++)
			for( iy=0; iy<ny; iy++) {
				SWAP( data[ix][iy], data[ix+ixmid][iy] );
			}

#undef SWAP
		return data;
	}
	static QSTEM::ComplexArray2DPtr ifftshift(QSTEM::ComplexArray2DPtr data) {
#define SWAP(a,b)       {t=a; a=b; b=t;}
		int ix, iy, ixmid, iymid;
		QSTEM::complex_tt t;
		...
		iymid = ny/2;
		if(nx % 2 != 0 || ny % 2!=0){
			throw std::runtime_error(string("cannot ifftshift an array of uneven size!"));
		}
		for( ix=0; ix<ixmid; ix++)
			for( iy=0; iy<ny; iy++) {
				SWAP( data[ix][iy], data[ix+ixmid][iy] );

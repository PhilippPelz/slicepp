...
void CUDAFunctions::phaseGrating(cufftComplex* V_d, int nAt, int nZ, float_tt* xyz_d, float_tt imPot, int* Z_d, int* Zlist, float_tt* occ_d, int s,
		int nx, int ny, int nSlices, float_tt dx, float_tt dy, float_tt dz) {
	...
	for (int j = 0; j < nZ; j++) {
		...
		putAtomDeltas <<< myGSize( nAt ), myBSize( nAt ) >>> ( V1_d, nAt, Z_d, Zlist[j], xyz_d, imPot, occ_d, s, nx, ny, nSlices, dx, dy, dz);
		createAtomicPotential <<< gS2D, bS >>> ( V2_d, Zlist[j], nx, ny, dx, dy);

...
void CExperimentTEM::Run()
{
	...
	timerTot = 0; /* cputim();*/
	DisplayProgress(-1);
	for (_runCount = 0;_runCount < m_avgRuns;_runCount++) {
		m_totalSliceCount = 0;
		...
		for (pCount = 0;pCount<m_cellDiv;pCount++) {
			...
			if (m_printLevel > 0) {
				printf("t=%gA, int.=%g (avgCount=%d)\n",
						m_thickness,m_intIntensity,_runCount);
			}

			/***************** FOR DEBUGGING ****************/
			if ((_runCount == 0) && (m_saveLevel >=0) && (pCount+1==m_cellDiv)) {
			...
			if ((m_tds) && (m_saveLevel > 2)) {
				...
				m_wave->WriteWave(_runCount, comment, params);
			}
#endif 
		}
		/////////////////////////////////////////////////////////////////////////////
					...
					for (ix=0;ix<m_nbout;ix++) {
						avgPendelloesung[ix][iy] =
								((float_tt)_runCount*avgPendelloesung[ix][iy]+
										m_pendelloesung[ix][iy])/(float_tt)(_runCount+1);
					}
				}
			}
			/***********************************************************
...
//			m_wave->ReadImage();
			...
			for (unsigned i=0;i<px;i++){
				...
				//               imageWave[i].real()*imageWave[i].real()+imageWave[i].imag()*imageWave[i].imag())/(float_tt)(_runCount+1);

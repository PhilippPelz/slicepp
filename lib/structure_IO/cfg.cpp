...
int CCfgReader::ReadAtoms(std::vector<atom> &atoms, std::vector<int> &uniqueAtoms,bool fillUnitCell)
{
	unsigned ncoord;
	...
	BOOST_LOG_TRIVIAL(trace)  << "Reading atoms from file ...";
	for (unsigned i=0; i<ncoord; i++)
	{
		...
		if(!thisZexists){
			...
			uniqueAtoms.push_back(atoms[i].Znum);
		}
	}
	return 0;
}

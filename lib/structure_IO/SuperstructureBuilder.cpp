...
//			free(sc);
			...
			for(int Znum : unique){
				bool thisZexists = (std::find(_superCell->uniqueZ.begin(), _superCell->uniqueZ.end(), Znum)) != _superCell->uniqueZ.end();
				if(false == thisZexists) _superCell->uniqueZ.push_back(Znum);
			}
